...
- `plotter_vision.drawing` owns drawing-program contracts, including capability checks,
  image-derived programs, portrait programs, and calibration-sheet programs. Camera-space overlays
  should be treated as views over explicit field/model artifacts, not as independent motion
  authority.
...
- The DrawingProgram calibration sheet is the `multi_shape_coordinate_sheet` family until a more
  specific sheet kind is added. It must be generated by `plotter_vision.drawing.capabilities`, then
  lowered by `plotter_vision.bridge.planner.build_polygon_draw_plan`; do not draw a separate Swift
  sheet overlay, do not use SVG import, and do not bypass planner simulation or projected preview.
...
```bash
rg -n "DrawingProgram|multi_shape_coordinate_sheet|build_polygon_draw_plan|residual_grid_v1|model_family|latest_drawing_calibration" plotter_vision macos tests README.md docs codex_prompts
```

Classify new model/program claims by runtime hits outside tests. `DrawingProgram` and
`multi_shape_coordinate_sheet` must appear in `plotter_vision.drawing` and bridge planning routes,
not only tests. `residual_grid_v1` must have non-test bridge/model/planner hits before docs call it
active; those hits now live in the drawing model, drawing observation routes, and optional
model-aware drawing planner stage.
